// Palavra Secreta — tela do celular. Cada um só vê a própria palavra.
(() => {
  let styled = false, aberto = false, lastRound = -1, ovKey = '';
  const style = `
    .ps-hid { border-radius:20px; background:linear-gradient(135deg,#0ea5e9,#1e3a8a); padding:34px 18px; text-align:center; font-size:22px; font-weight:900; line-height:1.3; }
    .ps-open { border-radius:20px; background:#0b0e17; border:3px solid #0ea5e9; padding:28px 18px; text-align:center; }
    .ps-open .w { font-size:40px; font-weight:900; line-height:1.15; word-break:break-word; }
    .ps-open .k { font-size:16px; color:var(--mut); font-weight:800; letter-spacing:2px; text-transform:uppercase; }
    .ps-imp { border-color:#ef4444 !important; }
    .ps-imp .w { color:#ef4444; }
    .ps-o { padding:12px 16px; border-radius:12px; background:#2a3350; font-weight:900; font-size:18px; text-align:center; }
    .ps-o.sel { background:#f59e0b; color:#111; }
    .ps-o.off { opacity:.35; }
    .ps-opt { display:flex; flex-wrap:wrap; gap:8px; }
    .ps-cl { display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:12px; background:#0b0e17; border:2px solid transparent; }
    .ps-cl.now { border-color:#facc15; }
    .ps-cl.dead { opacity:.4; }
    .ps-cl b { flex:1; text-align:right; font-size:20px; font-weight:900; word-break:break-word; }
    .ps-in { display:flex; gap:8px; } .ps-in input { flex:1; }
  `;
  ...
  function listaDicas(c, so) {
    ...
    return `<div class="box" style="display:flex;flex-direction:column;gap:8px"><p class="sub mut">Dicas ditas</p>
      ${(so || G.order).map(pid => { const p = ply(c, pid); if (!p) return '';
        ...
        return `<div class="ps-cl ${G.speaker === pid ? 'now' : ''} ${G.out.includes(pid) ? 'dead' : ''}">${c.nm(p)}<b>${cl.length ? cl.map(c.esc).join(' · ') : '<span class="mut" style="font-size:15px">—</span>'}</b></div>`;
      }).join('')}</div>`;
  }
  function minhaPalavra(c, curta) {
    const m = c.G.mine; if (!m) return '';
    if (!aberto) return `<div class="ps-hid" data-a="ver">👆 Toque para ver ${curta ? 'de novo' : 'sua palavra'}<br><small style="font-size:15px;opacity:.85">ninguém pode olhar sua tela</small></div>`;
    if (m.impostor) return `<div class="ps-open ps-imp" data-a="esconder"><div class="k">seu papel</div><div class="w">🕵️ VOCÊ É O IMPOSTOR</div>${m.hint ? `<p class="sub mut" style="margin-top:8px">Categoria: <b style="color:#fff">${c.esc(m.hint)}</b></p>` : '<p class="sub mut" style="margin-top:8px">Finja que sabe a palavra.</p>'}</div>`;
    return `<div class="ps-open" data-a="esconder"><div class="k">sua palavra</div><div class="w">${c.esc(m.word)}</div></div>`;
  }
  function placar(c) {
    const G = c.G;
  ...
  ARCADE.register('palavrasecreta', {
    phone: {
      ...
      html(c) {
        ...
        const m = G.mine || {};
        const opt = (a, vals, cur, fmt) => `<div class="ps-opt">${vals.map(v => `<div class="ps-o ${v === cur ? 'sel' : ''}" data-a="${a}" data-v="${v}">${fmt ? fmt(v) : v}</div>`).join('')}</div>`;

        if (G.phase === 'setup') {
          const cfg = G.cfg, poucos = c.C.players.length < 6;
          return `<div class="box center"><h2 style="font-size:26px">⚙️ Regras</h2><p class="sub mut" style="margin-top:6px">Qualquer um pode mudar. Vale para todos.</p></div>
            <div class="box"><p class="sub" style="margin-bottom:8px">Impostores</p>
              <div class="ps-opt"><div class="ps-o ${cfg.impostors === 1 ? 'sel' : ''}" data-a="cfgImp" data-v="1">1</div>
              <div class="ps-o ${cfg.impostors === 2 ? 'sel' : ''} ${poucos ? 'off' : ''}" data-a="cfgImp" data-v="2">2 ${poucos ? '(6+ jogadores)' : ''}</div></div></div>
            <div class="box"><p class="sub" style="margin-bottom:8px">O impostor recebe a categoria</p>
              <div class="ps-opt"><div class="ps-o ${cfg.hint ? 'sel' : ''}" data-a="cfgHint" data-v="1">Sim</div><div class="ps-o ${cfg.hint ? '' : 'sel'}" data-a="cfgHint" data-v="0">Não</div></div></div>
            <div class="box"><p class="sub" style="margin-bottom:8px">Modo Mister White <span class="mut">(o impostor recebe uma palavra parecida)</span></p>
              <div class="ps-opt"><div class="ps-o ${cfg.white ? 'sel' : ''}" data-a="cfgWhite" data-v="1">Ligado</div><div class="ps-o ${cfg.white ? '' : 'sel'}" data-a="cfgWhite" data-v="0">Desligado</div></div></div>
            ...
            <div class="box"><p class="sub" style="margin-bottom:8px">Categorias <span class="mut">(${cfg.cats.length})</span></p>
              <div class="ps-opt">${G.cats.map(k => `<div class="ps-o ${cfg.cats.includes(k.id) ? 'sel' : 'off'}" data-a="cfgCat" data-id="${k.id}">${k.emoji} ${esc(k.name)}</div>`).join('')}</div>
        ...
        if (G.phase === 'clues') {
          ...
          return `<div class="box center"><p class="sub mut">Palavra ${G.round} de ${G.rounds} · volta ${Math.floor(G.turn / Math.max(1, G.order.length)) + 1} de ${G.laps}</p>
            ...
            ${minha ? `<div class="box"><div class="ps-in"><input id="ps-clue" maxlength="20" placeholder="sua dica" autocomplete="off" autocapitalize="none" enterkeyhint="send"></div>
        ...
        if (G.phase === 'guess') {
          if (m.canGuess) return `<div class="box center"><div class="big-emoji">🎯</div><h2 style="font-size:24px;margin-top:8px">Chance final!</h2><p class="sub mut" style="margin-top:6px">Qual era a palavra secreta? Acertou, +1 ponto.</p></div>
            <div class="box"><input id="ps-guess" maxlength="40" placeholder="a palavra secreta" autocomplete="off" enterkeyhint="send"><button class="btn ok" style="margin-top:10px" data-a="chute">Chutar</button></div>
      ...
      after(c) {
        ...
        const i = document.getElementById('ps-clue');
        if (i && !i._ps) { i._ps = true; i.focus(); i.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); ARCADE.games.palavrasecreta.phone.act('dica', null, c); } }); }
        const g = document.getElementById('ps-guess');
        if (g && !g._ps) { g._ps = true; g.focus(); g.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); ARCADE.games.palavrasecreta.phone.act('chute', null, c); } }); }
      ...
      act(a, el, c) {
        ...
        switch (a) {
          ...
          case 'dica': { const i = document.getElementById('ps-clue'); const v = i && i.value.trim(); if (!v) return; if (/\s/.test(v)) return ARCADE.toast('Uma palavra só, sem espaço.'); i.value = ''; return send({ t: 'clue', text: v }); }
          ...
          case 'chute': { const i = document.getElementById('ps-guess'); const v = i && i.value.trim(); if (!v) return; i.value = ''; return send({ t: 'guess', text: v }); }
