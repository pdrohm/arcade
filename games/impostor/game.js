// Impostor. Todo mundo recebe a mesma palavra no celular, menos o(s) impostor(es).
// Uma dica por vez, discussão, votação e revelação na TV.
...
const wordOf = e => typeof e === 'string' ? e : (e && e.w) || '';
const hintOf = e => (e && typeof e === 'object' && e.h) || '';
const HINTS = {};                                // palavra -> dica (serve também para o modo Mister White)
for (const list of Object.values(WORDS)) for (const e of list) HINTS[wordOf(e)] = hintOf(e);
const MAX_IMP = n => Math.max(1, Math.floor((n - 1) / 2));   // nunca deixa impostores >= inocentes

module.exports = {
  meta: {
    id: 'impostor', name: 'Impostor', emoji: '🕵️‍♂️',
    tagline: 'Descubra quem não sabe a palavra.',
  ...
  create(api) {
    let s = {
      ...
      cat: null, word: '', hint: '', white: '', impostors: [], seen: [],
    ...
    function newRound() {
      ...
      if (s.cfg.white) {                                  // Mister White: o impostor recebe uma palavra parecida
        ...
        const pr = pool[Math.floor(Math.random() * pool.length)] || [wordOf(WORDS[cid][0]), wordOf(WORDS[cid][1])];
        s.word = pr[0]; s.white = pr[1]; s.hint = HINTS[pr[0]] || '';
      } else {
        let pool = (WORDS[cid] || []).filter(e => !s.used.includes(wordOf(e)));
        if (!pool.length) { s.used = []; pool = WORDS[cid] || []; }
        const pick = pool[Math.floor(Math.random() * pool.length)] || { w: 'Pizza', h: 'Comida redonda italiana' };
        s.word = wordOf(pick); s.hint = hintOf(pick); s.white = '';
      }
      s.used.push(s.word); if (s.used.length > 300) s.used.shift();
      s.impostors = shuffle(pids()).slice(0, nImp());
    ...
    const inst = {
      ...
      action(p, msg) {
        ...
        switch (msg.t) {
          case 'config': {                                // qualquer jogador muda as regras antes de começar
            ...
            const c = msg.cfg || {};
            if (c.impostors !== undefined) { const n = Math.round(Number(c.impostors)); s.cfg.impostors = (n >= 1 && n <= 4) ? n : 1; }
      ...
      view(me) {
        ...
        if (me) {                                          // parte privada: só o dono vê
          ...
          out.mine = s.phase === 'setup' ? null : {
            ...
            hint: imp && !s.cfg.white && s.cfg.hint ? s.hint : '',   // a dica é só do impostor, e só no modo "Com dica"
      ...
      serialize: () => ({ v: 2, s }),
      restore(d) {                                   // estado antigo (outra versão do jogo) é ignorado, sem quebrar o servidor
        if (!d || d.v !== 2 || !d.s || typeof d.s !== 'object' || Array.isArray(d.s)) return;
        const o = d.s;
        const FASES = ['setup', 'reveal', 'clues', 'discuss', 'vote', 'result', 'guess', 'scores', 'end'];
        if (!FASES.includes(o.phase)) return;
        if (!Array.isArray(o.impostors) || !Array.isArray(o.order) || !Array.isArray(o.out) || !o.cfg || typeof o.cfg !== 'object') return;
        s = { ...s, ...o };
        s.seen = Array.isArray(s.seen) ? s.seen : [];
        s.endVotes = Array.isArray(s.endVotes) ? s.endVotes : [];
        s.used = Array.isArray(s.used) ? s.used : [];
        s.clues = (s.clues && typeof s.clues === 'object') ? s.clues : {};
        s.votes = (s.votes && typeof s.votes === 'object') ? s.votes : {};
        s.scores = (s.scores && typeof s.scores === 'object') ? s.scores : {};
        s.hint = typeof s.hint === 'string' ? s.hint : '';
